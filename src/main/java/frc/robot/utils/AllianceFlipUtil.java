...
public class AllianceFlipUtil {
    ...
    public static Translation2d apply(Alliance alliance, Translation2d translation) {
        if (shouldFlip(alliance)) {
            return new Translation2d(Constants.FIELD_LENGTH - translation.getX(), translation.getY());
        } else {
            return translation;
        }
    }
    ...
    public static Pose2d apply(Alliance alliance, Pose2d pose) {
        if (shouldFlip(alliance)) {
            return new Pose2d(
                    Constants.FIELD_LENGTH - pose.getX(),
                    Constants.FIELD_WIDTH - pose.getY(),
                    pose.getRotation());
        } else {
            return pose;
        }
    ...
    public static Trajectory.State apply(Alliance alliance, Trajectory.State state) {
        if (shouldFlip(alliance)) {
            return new Trajectory.State(
                    ...
                    new Pose2d(
                            Constants.FIELD_LENGTH - state.poseMeters.getX(),
